...
void addBalanceState(Manager *manager, local_id id, BalanceState balanceState) {
    ...
    // прибовляю с [x, y), где y последний добавленный элемент
} 
...
void restoreBalanceState(Manager *manager, local_id id, timestamp_t newTime) {
    ...
    if (manager->pid[id].balance.s_time + 1 == newTime) {
        return;
    }

    manager->pid[id].balance.s_time++;
    timestamp_t t = manager->pid[id].balance.s_time;
    // printf("%d != %d\n", t, newTime);
    while (t != newTime) {
        manager->pid[id].balance.s_time = t;
        manager->pid[id].history.s_history[(*len)] = manager->pid[id].balance;
        (*len)++;
        t++;
        // printf("len++ -> %d\n", (*len));
...
void restorePendingBalance(BalanceHistory *history, timestamp_t sendTime, timestamp_t currentTime, balance_t balance) {
    ...
    // printf("restore pending balance\n");
    sendTime--;
    while (sendTime < currentTime) {
        // printf("SENDTIME: %d -> CURRENTTIME: %d\n", sendTime, currentTime);
        history->s_history[sendTime].s_balance_pending_in = balance;
        sendTime++;
    }
}
