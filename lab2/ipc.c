...
int receive_any(void * self, Message * msg) {
    ...
    for (int i = 0; i < manager->chanLen; i++) {
        if (manager->chan[i].FromTo[1] == id) {
            ...
            int flags = fcntl(manager->chan[i].PipesFd[0], F_GETFL);  // Получаем текущие флаги
            if (flags == -1) {
                perror("fcntl(F_GETFL)");
                return -1;
            }
            if (fcntl(manager->chan[i].PipesFd[0], F_SETFL, flags | O_NONBLOCK) == -1) {
            ...
            // if (bytes_read > 0) {
            //     printf("GOT HEADER BY PROC: %d -> HEADER: len: %d type: %d\n",id, msg->s_header.s_payload_len, msg->s_header.s_type);
            // }
